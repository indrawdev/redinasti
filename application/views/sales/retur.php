        ...
        <div class="row-fluid suppptrans-detail">
            <div class="span8">
                <fieldset>
                    ...
                    <div class="row-fluid">
                        <div class="span12">
                            <label for="id_store">Toko</label>: <strong><?=$record['store']?></strong>
                        </div>
                    </div>
                    <div class="row-fluid">
                        <div class="span12">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="tableRetur">
                            ...
                            <?php $total_qty=0; $i=0; foreach ($retur as $rtr) : ?>
                                <tr id="rowRetur<?=$i?>">
                                    ...
                                    <td class="division"><?=$rtr['division']?></td>
                                    ...
                                    <tr id="row<?=$i?>">
                                        <td class="productOption">
                                            ...
                                            <?php foreach ($productions as $production) : ?>
                                                <option value="<?=$production['id_sales_production']?>" <?=(isset($prod['id']) && $prod['id'] == $production['id_sales_production']) ? 'selected="selected"' : '' ?>><?=$production['item_category']?> (<?=$production['production_code']?>) (<?=$production['item_name']?>)</option>
                                            <?php endforeach; ?>
                                            </select>
                                        </td>
...
<script type="text/javascript">
    ...
    function addProduction() {
        ...
        <?php foreach ($productions as $production) : ?>
            html += '<option value="<?=$production['id_sales_production']?>"><?=$production['item_category']?> (<?=$production['production_code']?>) (<?=$production['item_code']?>)</option>';
        <?php endforeach; ?>
        html += '</select>';
        html += '</td>';
