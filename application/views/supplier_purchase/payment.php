...
<section class="well animated fadeInUp">
        ...
        <div class="row-fluid">
            <div class="span8">
                <fieldset>
                    ...
                    <div class="row-fluid row-label">
                        ...
                        <?php if ($supplier_credit) : ?>
                        <div class="row-fluid row-label">
                            <div class="span12">
                                <label for="total_credit">Total Piutang:</label> <strong>Rp. <?=myprice($supplier_credit)?></strong>
                            </div>
                        </div>
                        <?php endif; ?>
                    <?php endif; ?>
                    ...
                    <div class="genPayment">
                        <table class="table table-striped table-bordered table-hover" id="tablePayment">
                            ...
                            <?php if (isset($post['post_payment']) && count($post['post_payment'])>0) : $a=0; $total_payment=0; ?>
                                <?php foreach ($post['post_payment'] as $pay) : ?>
                                    <tr id="rowPay<?=$a?>">
                                        <td class="productOption">
                                            <select name="post_payment[<?=$i?>][type]" id="post_payment_type_<?=$a?>" class="paySelect" data-id="<?=$a?>">
                                ...
                                <?php $i++; $total_payment += $pay['price']; endforeach; ?>
                            <?php endif; ?>
            ...
            <div class="span4">
                ...
                <fieldset>
                    ...
                    <div class="row-fluid">
                        <div class="span12">
                            <label for="cal">Kalkulasi Pembayaran</label>
                            <div id="calc">
                                Total yang harus dibayar: <br/>
                                <span class="total-price" data-value="<?=$record['total_price']?>"><?=myprice($record['total_price'])?></span><span class="total-paid" data-value="<?=$total_paid?>"> - <?=myprice($total_paid)?></span> = <span id="total-all">0</span>
                            </div>
                        </div>
                    </div>
                </fieldset>
    ...
    <script type="text/javascript">
        ...
        function calculate() {
            var totalprice = $('.total-price').attr('data-value'),
                totalpaid = $('.total-paid').attr('data-value');
            $('#total-all').html(number_format(totalprice-totalpaid,0,',','.'));
        }
        /**
        $(function(){
        ...
        */
        ...
        function addPayment() {
            ...
            html += '</select>';
            html += '</td>';
            html += '<td class="price">Rp. <input type="text" name="post_payment['+ row +'][price]" id="post_payment_price_'+ row +'" value="" class="input-text number_only price_giro" readonly="readonly"/></td>';
            ...
            $("#post_payment_type_"+ row).select2({placeholder: "Pilih Produk"}).on('change', function(e) {
                var this_id = $(this).attr('data-id');
                var giro_id = e.val,
                ...
                if (giro_id != 0) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data['value']['giro_price']) {
                                $("#post_payment_price_"+seltor).val(data['value']['giro_price']).attr('readonly',true);
                                var jumlah1 = data['value']['giro_price'];
                            }
                        }
                    });
                } else {
                    ...
                    var jumlah1 = 0;
                    $("#post_payment_price_"+seltor).keyup(function() {
                        jumlah1 = this.value;
                    });
                }
                var jumlah1 = $("#post_payment_price_"+seltor).val();
                $('.price_giro').each(function() {
                    jumlah1 += parseInt($(this).val());
                });
                console.log(jumlah1);
                //alert (jumlah1);
                var jumlah2 = '<?=$total_paid?>',
                    jumlah = parseInt(jumlah1)+parseInt(jumlah2);
                $('.total-paid').attr('data-value',jumlah).html(' - '+number_format(jumlah,0,',','.'));
            });
